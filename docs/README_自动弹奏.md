...
```bash
python play.py --song "D:/音乐/某首歌.mp3" --countdown 8 --debug-timing
python play.py --song "D:/音乐/某首歌.mp3" --dry-run            # 先看听成了什么
python play.py --song "D:/音乐/某首歌.mp3" --mp3-save 我的歌     # 存成 songs/我的歌.jianpu
```

流程：`score.parse_any()` 按扩展名分发（`.mp3/.wav/.ogg/.flac/.m4a/.aac/.opus/.wma`）→ `audio_in.read_song()`：

1. PyAV 解码成单声道 22050Hz（**不需要单独装 ffmpeg/sox**）
2. 带通 120~1500Hz（滤掉鼓、低音、镲片；下限要盖住游戏最低可弹的 C3=131Hz）
3. 逐帧 **YIN** 基频（差分函数 + 累积均值归一 + 绝对阈值 + 抛物线插值；自相关走 FFT，分块向量化）
4. 音量门限 + 置信度门限 → 只有真在发声的帧作数
5. 相邻同音高并成一个音；短于 `--mp3-min-dur`（默认 0.08s）当噪声丢掉
6. BPM：`--bpm` 给了就用，没给从起音间隔估计（60~200BPM 网格打分）
7. 量化到 1/16 网格 → `(midi, 起拍, 时值)`，之后完全复用现有管线（按键映射、时序、五道缺音防线）

参数：

| 参数 | 默认 | 说明 |
|---|---|---|
| `--mp3-fmin` / `--mp3-fmax` | 120 / 1500 Hz | 只认这个区间内的音；人声旋律可试 200~1000 |
| `--mp3-conf` | 0.55 | 音高置信度门限，调高更保守、误判更少 |
| `--mp3-min-dur` | 0.08 秒 | 短于此的音当噪声丢掉 |
| `--mp3-save` | 无 | 把听出来的简谱存到 `songs/<名字>.jianpu` |

**效果边界（别期待过高）**：单声部/旋律突出的音频（清唱、单乐器、口琴录音）听得准；
编曲厚重的流行歌会跟着最突出的乐器走。自测 `python dev/selftest4.py` 会合成一段已知旋律 →
编成 MP3/WAV → 听回来对答案（可复现、不含任何版权音频）。
